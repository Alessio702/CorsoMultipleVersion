...
<body>
 <form method = "post" action = "/WebHbCorso/CtrlGestioneCorso" name = "GestioneCorso">
  <p>&nbsp;&nbsp; <input type="submit" name="cmdAzione" value="Torna alla Home">
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <font size="+3">Gestione Corsi</font>
  </p>
  <% List pgsElenco = (List) request.getAttribute("elencoCorsi");
  ...
  <table border = "1">
   <tbody>
    ...
    <%while(i < numeroRighe)
     ...
     <tr>
      <td>
      <input type="radio" name="rdoIDCorso" value="<%= ((Corso)pgsElenco.get(i)).getId() %>" checked = "checked"> 
      </td>
      <td> <%= ((Corso)pgsElenco.get(i)).getCorso() %></td>
      <td> <%= ((Corso)pgsElenco.get(i)).getoDocente().getCognome() %></td>
      <td> <%= ((Corso)pgsElenco.get(i)).getoDocente().getNome() %></td>
